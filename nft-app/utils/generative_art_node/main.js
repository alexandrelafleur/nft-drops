...
const buildDir = `${process.env.PWD}/utils/generative_art_node/build`;
const metDataFile = '_metadata.json';
const layersDir = `${process.env.PWD}/utils/generative_art_node/layers`;
...
const drawLayer = async (_layer, _edition, random) => {
  const rand = random();
  console.log(rand)
  let element =
...
const createFiles = async (edition, rand) => {
  const layers = layersSetup(layersOrder);

  // for (let i = 1; i <= edition; i++) {
  await layers.forEach(async (layer) => {
    await drawLayer(layer, edition, rand);
  });
  addMetadata(edition);
  // if (i % 100 == 0) {
